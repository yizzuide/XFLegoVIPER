...
我想要的框架是不需要学太多流程式API，提供的关键API使用简单，不依赖其它第三方库，除了拆分代码层次而不需要学习过多其它编码思维，代码的运行流程是可见可测的，兼容旧项目中其它架构并与之融合共处，各架构的模块之间有通信机制，兼容XIB/Storyboard/纯代码界面,并可以扩展出与项目业务关系的核心模块。
