...
我想要的框架是不需要学太多流程式API，提供的关键API使用简单，不依赖其它第三方库，除了拆分代码层次而不需要学习过多其它编码思维，代码的运行流程是可见可测的，兼容旧项目中其它架构并与之融合共处，各架构的模块之间有通信机制，兼容XIB/Storyboard/纯代码界面,并可以扩展出与项目业务关系的核心模块。于是就写了这个作为国内唯一款且接地气的VIPER框架！这个框架的早期版本就在我上家公司的一个业务复杂的模块实践过，现在的版本功能更加强大、更加完善，希望大家喜欢！
